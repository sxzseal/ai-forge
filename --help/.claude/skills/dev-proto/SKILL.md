...
| 提取内容 | 来源章节 | 用途 |
|----------|---------|------|
| 用户故事 | Section 4 | 确定需要哪些组件和交互 |
| 功能需求 | Section 5 | 确定组件功能边界 |
| API 契约 | Section 7 | 生成 MSW handlers |
| UI 规格 | Section 8 | 确定页面结构和组件拆分 |
| 组件拆分表 | Section 8 | 确定要创建的组件列表 |
...
基于 PRD Section 7 的 API 契约，生成 MSW handlers。
