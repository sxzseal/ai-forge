...
- 「生成测试」「写测试」「跑测试」
- 「自动测试」「dev-test」
- 被 `/dev-loop` 作为 Phase 5 调用

**前置条件**：

- `.loop/prd.md` 存在（包含验收标准）
- 代码已实现（Phase 3 完成）
...
读取 `.loop/prd.md`，提取 Section 4 的所有验收标准（AC-xxx 条目）。

同时读取 **`.loop/prototype/stories-manifest.md`**（如果存在），提取 Story 文件列表与 PRD 用户故事映射关系，用于在 Step 2 为每条 AC 标注关联的 Story。
...
```json
{
  "currentPhase": "deploy",
  "phases": {
    "test": { "status": "completed", "completedAt": "<ISO timestamp>" }
  },
  "artifacts": {
    "testScenarios": ".loop/test/scenarios.md",
    "testCoverage": ".loop/test/coverage-report.md"
  }
}
```
